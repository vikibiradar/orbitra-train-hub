...
export function PlanFinalEvaluationModal({
  ...
  const watchOtherMembers = [
    form.watch("otherPanelMember1"),
    form.watch("otherPanelMember2"),
    form.watch("otherPanelMember3"),
    form.watch("otherPanelMember4"),
    form.watch("otherPanelMember5"),
  ].filter(Boolean);
  ...
  const handleOtherPanelMemberChange = (index: number, value: string) => {
    ...
    const newSelectedMembers = [
      form.getValues("otherPanelMember1"),
      form.getValues("otherPanelMember2"),
      form.getValues("otherPanelMember3"),
      form.getValues("otherPanelMember4"),
      form.getValues("otherPanelMember5"),
    ...
    if (!watchMainPanelMember && newSelectedMembers.length > 0) {
      ...
      const memberNames = newSelectedMembers
        .map(id => mockPanelMembers.find(m => m.id === id)?.name)
        .filter(Boolean)
        .join(", ");
  ...
  const onSubmit = (data: FormValues) => {
    ...
    const allPanelMembers = [
      data.mainPanelMember,
      data.otherPanelMember1,
      data.otherPanelMember2,
      data.otherPanelMember3,
      data.otherPanelMember4,
      data.otherPanelMember5,
    ].filter(Boolean);

    const panelMemberNames = allPanelMembers
      .map(id => mockPanelMembers.find(m => m.id === id)?.name)
      .filter(Boolean)
      .join(", ");
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {getAvailablePanelMembers(field.value).map((member) => (
                  ...
                  return (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Select
                            onValueChange={(value) => handleOtherPanelMemberChange(index, value)}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="Enter comments (comma-separated list of panel members if needed)"
                        className="resize-none"
                        rows={3}
                        {...field}
                      />
                    </FormControl>
                    {watchOtherMembers.length > 0 && (
                      <p className="text-sm text-muted-foreground">
                        Selected members: {watchOtherMembers.map(id => 
                          mockPanelMembers.find(m => m.id === id)?.name
                        ).filter(Boolean).join(", ")}
                      </p>
                    )}
                    <FormMessage />
