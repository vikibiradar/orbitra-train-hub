...
export function SourcePlannerSelector({
  ...
  const availablePlanners = useMemo(() => {
    ...
    return allPlanners.filter((planner) => {
      ...
      if (planner.plannerType === 11) { // PlannerType.GENERAL_NEW_EMPLOYEE
      ...
      if (planner.plannerType === 20) { // PlannerType.ANNUAL_EMPLOYEE
        const plannerYear = new Date(planner.createdDate).getFullYear();
        return (
          (plannerYear === currentYear || plannerYear === previousYear) &&
  ...
  const filteredPlanners = useMemo(() => {
    ...
    return availablePlanners.filter(
      (planner) =>
        planner.plannerNumber.toLowerCase().includes(query) ||
        planner.employee.firstName.toLowerCase().includes(query) ||
        planner.employee.lastName.toLowerCase().includes(query) ||
        planner.employee.employeeCode.toLowerCase().includes(query)
    );
